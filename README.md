...
一度切り替えると、同じ起動中に次に開くファイル・別タブ・差分にも設定を引き継ぐ。
差分一覧でファイルを選んで `Enter` を押すと本文へフォーカスが移り、そのまま `j` / `k` で読める。
本文からは `Space h` で差分一覧へ直接戻る。左隣の差分へ移る場合は `Ctrl h` を使う。
...
対応言語では `()`・`[]`・`{}` などを入れ子ごとに色分けし、対応する括弧を同色で表示する。
文字列内の括弧は対象外。`rainbow-delimiters.nvim` は実行時コードを監査した
コミットに固定している。Clojure・Kotlin・Java・Lua・Python・TypeScriptなど、
パーサーと括弧用queryが揃っている言語で有効になる。
