...
差分を見ている間は、下部バーにも状態に合わせて
「一覧を隠す `Space e`」または「一覧を表示 `Space e`」が出る。クリックでも切り替えられる。

herdr 側で Neovim のペイン幅を変えたときは、左のファイル一覧幅を保ったまま、
左右の差分が同じ幅になるよう自動で調整される。
