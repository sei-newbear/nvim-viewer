...
### 3.1 `~/.config/nvim` には入れないこと

**既存の nvim があるかどうかに関わらず、`~/.config/nvim-viewer` に入れる。**
...
**何も入っていないマシンでも同じ。** `~/.config/nvim` を空けておけば、
後から普通の nvim を使いたくなったときに困らない。
...
### 5.9 `herdr pane send-text` は任意のペインへ送れる

`agent send` は herdr 0.7.5 で廃止された。`herdr pane send-text` は任意のペインに
Enter なしで送れるため、送信先の解決結果が pane_id であるこの連携では常にこちらを使う。
...
素性不明のものは無い。`diffview.nvim` が GPL である点は、この設定が
そのコードを同梱せず利用者が自分で取得する形なので、この設定自体の
ライセンス選択を縛らない。
